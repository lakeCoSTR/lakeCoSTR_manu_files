...
temp_monthly_median_kurtosis %>% 
  ...
  # geom_smooth(method = 'lm', se = F, inherit.aes = T) +
  final_theme
# ggsave(file.path(figdir, 'monthly_patterns_calib_temp_kurtosis.png'), height = 3, width = 9)
...
#set up png device
jpeg(file.path(figdir, 'FigH_application_monthly_median_temp_cloud_together.jpg'),
...
source_contrasts <- contrasts(temp_monthly_median_kurtosis$source)
