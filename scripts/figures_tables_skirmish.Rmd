...
insitu <- read_csv(paste0(dir, 'data/all_temp_data_v2021Apr21.csv'),
                   col_types = c('')) %>% 
...
length(unique(hf_insitu$lat_dd))
length(unique(hf_insitu$long_dd))


#number of sites
lf_insitu <- insitu %>% 
  filter(frequency == 'monthly')
length(unique(lf_insitu$lat_dd))
length(unique(lf_insitu$long_dd))
```
Table of mean temperature per month

```{r}
hf_meantemp_permonth_year <- hf_insitu %>% 
  mutate(year = as.numeric(format(datetime, '%Y'))) %>% 
  group_by(month, year, location) %>% 
  summarize(mean_depth_m = mean(depth_m),
            mean_temp_degC = mean(temp_degC))

ggplot(hf_meantemp_permonth_year, aes(x = year, y = mean_temp_degC)) +
  geom_point(aes(color = location, size = mean_depth_m)) +
  labs(y = 'mean monthly temperature (degrees C)') +
  facet_grid(month ~ ., scales = 'free_y') +
  scale_color_manual(values = c("#000000", '#999999', "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")) +
  final_theme
ggsave(paste0(figdir, 'monthly_average_labels.png'), height = 6, width = 9, dpi = 200)
```
