...
#plot to make sure prediction is correct
ggplot(C2_validation_IQR, aes(x = temp_med, y = pred_IQR))+
  geom_point(color = 'blue') +
  geom_point(aes(x = temp_med, y = surface_temp_median))
```
...
```{r}
ggplot(C2_lst_all_val, aes(x = calib_range, y = calib_IQR)) +
  geom_point() +
  geom_abline(intercept =  0, slope = 1) +
  final_theme
...
ggplot(C2_validation_range, aes(sample = opt_resid)) +
  geom_qq() +
  geom_qq_line()

ggplot(C2_validation_range, aes (y = opt_resid, x = temp_med)) +
  geom_point() +
