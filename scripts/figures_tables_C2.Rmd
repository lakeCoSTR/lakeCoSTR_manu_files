...
## predicted v observed by max range - median value
```{r}
...
C2_lst_all_val <- C2_lst_all_val %>% 
  select(-calib_range) %>% 
  full_join(., C2_calibration_range)

```


## predicted v observed by max range - max and min value
```{r}
...
ggplot(C2_validation_range, aes(x = max_temp, y = surface_temp_max)) +
  ...
  final_theme
ggsave(paste0(figdir, 'C2_deming_range_maxtemp_v11Oct2021.jpg'), height = 5, width = 5, units = 'in')
...
ggplot(C2_validation_range, aes(x = min_temp, y = surface_temp_min)) +
  ...
  final_theme
ggsave(paste0(figdir, 'C2_deming_range_mintemp_v11Oct2021.jpg'), height = 5, width = 5, units = 'in')
...
C2_lst_all_val <- C2_lst_all_val %>% 
  select(-calib_IQR) %>% 
  full_join(., C2_calibration_IQR)


```


## predicted v observed by max IQR - max and min value
```{r}
...
ggplot(C2_validation_IQR, aes(x = min_temp, y = surface_temp_min)) +
  ...
  labs(y = 'satellite-derived min\nskin temperature (deg C)', 
       x = expression(paste(italic('in-situ'), ' min water temp (deg C)'))) +
  final_theme
ggsave(paste0(figdir, 'C2_deming_IQR_mintemp_v11Oct2021.jpg'), height = 5, width = 5, units = 'in')
