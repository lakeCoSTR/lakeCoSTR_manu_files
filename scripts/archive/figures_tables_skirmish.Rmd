...
updated 19Jul2021 to export the scenes that passed QAQC
updated 11Aug2021 with residuals for azimuth and sun angle, etc
updated 30Sept2021 with C2 data for comparison
...
## Collectoin 2 data
```{r}
C2 <- read_csv(file.path(datadir, 'colab-output/C2/sunapee_1985_2020_C2_stats.csv')) %>% 
...
C2_filt_mod <- C2_filt_freeze %>% 
  left_join(., modal_analysis) %>% 
  filter(unimodal == 'unimodal')

C2_filt_maxrange <- C2_filt_mod %>% 
  mutate(temp_spread = round(surface_temp_max, digits = 1) - round(surface_temp_min, digits = 1)) %>% 
  filter(temp_spread < max_spread*1.1)

C2_filt_maxIQR <- C2_filt_mod %>% 
  mutate(IQR = round(surface_temp_p75, digits = 1) - round(surface_temp_p25, digits = 1))  %>% 
  filter(IQR < IQR*1.1)
...
validation_C2 <- C2_filt_maxrange %>% 
  select(date, LSmission) %>% 
  inner_join(., C2_paired)
...
#### predicted v observed by range C2
```{r}
val_C2_med_ls <- lm(validation_C2$surface_temp_median ~ validation_C2$temp_med)
summary(val_C2_med_ls)

val_C2_med_dem <-deming::deming(validation_C2$surface_temp_median ~ validation_C2$temp_med)
val_C2_med_dem

cor(validation_C2$surface_temp_median, validation_C2$temp_med)

ggplot(validation_C2, aes(x = temp_med, y = surface_temp_median)) +
  geom_point() +
  geom_abline(intercept = val_C2_med_dem$coefficients[1], slope = val_C2_med_dem$coefficients[2], color = 'blue') +
  geom_abline(intercept = val_C2_med_dem$ci[1,1], slope = val_C2_med_dem$ci[2,1], color = 'blue', linetype = 2) +
  geom_abline(intercept = val_C2_med_dem$ci[1,2], slope = val_C2_med_dem$ci[2,2], color = 'blue', linetype = 2) +
  geom_abline(intercept = 0, slope = 1) +
  coord_cartesian(xlim = c(7,27),
                  ylim = c(7,27)) +
  labs(y = 'satellite-derived median\nskin temperature C2 (deg C)', 
       x = expression(paste(italic('in-situ'), ' median water temp (deg C)'))) +
  final_theme
ggsave(paste0(figdir, 'deming_reg_C2_median_v30Sept2021.jpg'), height = 5, width = 5, units = 'in')

validation_C2$pred_temp = val_C2_med_dem$coefficients[1] + val_C2_med_dem$coefficients[2]*validation_C2$temp_med

lst_insitu_dem_forresid = mcreg(x = validation_C2$temp_med, y = validation_C2$surface_temp_median, method.reg = 'Deming')
validation_C2$opt_resid = MCResult.getResiduals(lst_insitu_dem_forresid)$optimized

#plot to make sure prediction is correct
ggplot(validation_C2, aes(x = temp_med, y = pred_temp))+
  geom_point(color = 'blue') +
  geom_point(aes(x = temp_med, y = surface_temp_median))
```
...
#### apply calibration of temp to LS-derived data ####
```{r}
C2_filt_maxrange <- C2_filt_maxrange %>% 
  mutate(calib_med_temp_degC = (surface_temp_median-val_C2_med_dem$coefficients[1])/val_C2_med_dem$coefficients[2])
C2_filt_maxrange

C2_filt_maxrange %>% 
  select(date, LSmission, temp_spread, pct_lake, surface_temp_max:surface_temp_stdDev, calib_med_temp_degC) %>% 
  write.csv(file.path(datadir, 'LS_C2_passQAQC_v31Sept_2021.csv'), row.names = F)

#plot to make sure calibration is correct
C2_filt_mod_select_range_calib <- C2_filt_maxrange %>% 
  select(date:temp_spread, calib_med_temp_degC)

validation_C2_calib <- C2_paired%>% 
  inner_join(., C2_filt_mod_select_range_calib)

ggplot(validation_C2_calib, aes(x = temp_med, y = surface_temp_median)) +
  geom_point() +
  geom_point(aes(x = temp_med, y = calib_med_temp_degC), color = 'red')+
  geom_abline(intercept = val_C2_med_dem$coefficients[1], slope = val_C2_med_dem$coefficients[2], color = 'blue') +
  geom_abline(intercept = val_C2_med_dem$ci[1,1], slope = val_C2_med_dem$ci[2,1], color = 'blue', linetype = 2) +
...
#### predicted v observed 50% cutoff
```{r}
