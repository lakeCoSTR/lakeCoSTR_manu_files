...
# Collection 2 data
C2ST <- read_csv(file.path(C2_datadir, 'sunapee_v1_12_temp_landsat_paired.csv')) %>% 
  mutate(date = as.Date(substrRight(`system:index`, 8), format = '%Y%m%d')) %>% 
  mutate(LSmission = case_when(grepl('LT05', `system:index`) ~ 'LS 5',
                               grepl('LT04', `system:index`) ~ 'LS 4',
                               grepl('LE07', `system:index`) ~ 'LS 7',
...
# by pixel count
perclake <- ggplot(C2ST_kurtosis, aes(x = (availablePixels_count/17134)*100, y = opt_resid)) +
  geom_point() +
  # geom_line(aes(group = date)) +
...
# by kurtosis cover
cloud <- ggplot(C2ST_kurtosis, aes(x = cloudcover_pct_scene, y = opt_resid)) +
  geom_point() +
  # geom_line(aes(group = date)) +
  geom_abline(intercept =  0, slope = 0) +
  coord_cartesian(ylim = c(-5, 5)) +
...
# by sun elevation
sunelev <- ggplot(C2ST_kurtosis, aes(x = sunelev_deg_scene, y = opt_resid)) +
  geom_point() +
  # geom_line(aes(group = date)) +
  geom_abline(intercept =  0, slope = 0) +
  coord_cartesian(ylim = c(-5, 5)) +
...
esd <- ggplot(C2ST_kurtosis, aes(x = esd_au_scene, y = opt_resid)) +
  geom_point() +
  # geom_line(aes(group = date)) +
  geom_abline(intercept =  0, slope = 0) +
  coord_cartesian(ylim = c(-5, 5)) +
...
# by azimuth
azi <- ggplot(C2ST_kurtosis, aes(x = sunazi_deg_scene, y = opt_resid)) +
  geom_point() +
  # geom_line(aes(group = date)) +
  geom_abline(intercept =  0, slope = 0) +
  coord_cartesian(ylim = c(-5, 5)) +
...
write.csv(month_error, file.path(fig_dir, 'A1ST8_deming_kurtosis_performancebymonth.csv'), row.names = F)
