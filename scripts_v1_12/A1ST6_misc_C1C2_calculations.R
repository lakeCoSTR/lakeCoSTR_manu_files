...
C2 <- read.csv(file.path(C2_datadir, 'sunapee_v1_12_temp_stats.csv')) %>% 
  mutate(date = as.Date(substrRight(`system.index`, 8), format = '%Y%m%d')) %>% 
  mutate(LSmission = case_when(grepl('LT05', `system.index`) ~ 'LS 5',
                               grepl('LT04', `system.index`) ~ 'LS 4',
                               grepl('LE07', `system.index`) ~ 'LS 7',
...
sum(tableb$C2)
sum(tableb$kurtosis)
round(((sum(tableb$C2)-sum(tableb$C1))/sum(tableb$C1))*100, digits = 1)
round(((sum(tableb$kurtosis)-sum(tableb$none))/abs(sum(tableb$none)))*100, digits = 1)
